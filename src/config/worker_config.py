...
def get_optimal_num_workers(file_path: Path) -> int:
    """Calculates the optimal number of workers based on CPU count, RAM availability, and file size."""
    ...
    # File size base limit: 1 worker per 20MB
    bytes_per_worker = 20 * 1024 * 1024
    max_by_file_size = max(1, file_size_bytes // bytes_per_worker)

    # Optimal count combines CPU, RAM and file scale constraints
    workers = min(num_cpu, max_by_memory, max_by_file_size)
