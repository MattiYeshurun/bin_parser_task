...
FORMAT_MAP: Dict[str, Tuple[str, int, Optional[float]]] = {
    ...
    'M': ('b', 1, None), 'q': ('q', 8, None), 'Q': ('Q', 8, None), # Q נשאר שלם ארוך, ללא שום מכפילים!
}
...
class BinParserService:
    ...
    def _load_message_formats(self) -> None:
        ...
        with self.file_path.open('rb') as f:
            if self.file_path.stat().st_size < 90: 
                return
    ...
    def decode_message(self, payload: memoryview, type_id: int) -> Optional[Message]:
        ...
        if not message_format or not message_format.struct_obj: 
            return None
        ...
        fields: Dict[str, Any] = {}
        # פירסור ואינטגרציה מדויקת של סוגי השדות והמכפילים שלהם
        for index, value in enumerate(raw_values):
            fmt_char = message_format.format_string[index] if index < len(message_format.format_string) else ''
            ...
            if index in getattr(message_format, 'array_a_indices', []):
                if isinstance(value, (bytes, bytearray, memoryview)):
                    ...
                    value = arr
            elif fmt_char in ('n', 'N', 'Z') or isinstance(value, (bytes, bytearray, memoryview)):
                # השארת שדה ה-Data של FILE כבייטים גולמיים אם זה פורמט Z כדי להתאים ל-pymavlink
                if fmt_char == 'Z' and message_format.name == 'FILE':
                    value = bytes(value)
                else:
                    value = _null_term(value)

            # הצמדת המכפיל הנכון אך ורק לשדות שדורשים נקודה צפה (כמו c, C, e, E, L)
            multiplier = message_format.multipliers[index] if index < len(message_format.multipliers) else None
            if multiplier is not None and fmt_char in ('c', 'C', 'e', 'E', 'L'):
